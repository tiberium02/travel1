...
class Wp_Travel_Engine_Admin {
	...
	public function prepare_filter_params( $force = false ) {
		if ( 'done' === get_option( 'wte_search_params_updated_510', false ) && ! $force ) {
			return;
		}
		...
		update_option( 'wte_search_params_updated_510', 'done', true );
	}
	...
	public function wp_travel_engine_settings_page() {
		add_submenu_page(
			...
			__( 'WP Travel Engine Admin Settings', 'wp-travel-engine' ),
			__( 'Settings', 'wp-travel-engine' ),
			'manage_options',
			basename( __FILE__ ),
			array( $this, 'wp_travel_engine_callback_function' )
		);
	}
		...
		if ( get_post_type( $post ) === 'booking' ) {
			...
			<div class="misc-pub-section misc-pub-booking-status">
				...
				if ( ( 'refunded' === $label_key || 'canceled' === $label_key ) ) {
					...
					?>
					<label for="wp_travel_engine_booking_status"><?php esc_html_e( 'Booking Status', 'wp-travel-engine' ); ?></label>
					<select id="wp_travel_engine_booking_status" style="margin:10px;padding:10px;font-weight:700;color:#ffffff;background-color:<?php echo esc_attr( $status[ $label_key ]['color'] ); ?>" name="wp_travel_engine_booking_status" >
					...
					;(function(){
						var status = %s;
						var statusSelector = document.getElementById( 'wp_travel_engine_booking_status' )
						statusSelector && statusSelector.addEventListener('change', function(e) {
							this.style.setProperty('background - color', status[this.value] ? status[this.value].color : this.style.backgroundColor)
						})
					})();";

					wp_add_inline_script( 'wp-travel-engine', sprintf( $js_string, wp_json_encode( $status ) ), 'after' );
					?>
					<?php
				}
				?>
	...
	*/
	...
	public function wp_travel_engine_extensions_page() {
		add_submenu_page( 'edit.php?post_type=booking', __( 'Extensions for WP Travel Engine.', 'wp-travel-engine' ), __( 'Extensions', 'wp-travel-engine' ), 'manage_options', 'extensions', array( $this, 'wp_travel_engine_extensions_callback_function' ) );
	}
	...
	public function wp_travel_engine_themes_page() {
		add_submenu_page( 'edit.php?post_type=booking', __( 'Themes for WP Travel Engine.', 'wp-travel-engine' ), __( 'Themes', 'wp-travel-engine' ), 'manage_options', 'themes', array( $this, 'wp_travel_engine_themes_callback_function' ) );
	}
					...
					if ( ! empty( $sub_tabs ) ) :
						?>
							...
							foreach ( $sub_tabs as $key => $tab ) :
								$class_names = '';
								if ( in_array( $key, array( 'currency', 'booking_notifications', 'purchase_receipt' ), ! 0 ) ) {
									$class_names = 'wte_note_430';
								}
								?>
									<a href="javascript:void(0);" data-wte-update="<?php echo esc_attr( $class_names ); ?>" class="wpte-tab <?php echo esc_attr( $key ); ?> <?php echo 1 === $current ? 'current' : ''; ?>"><?php echo esc_html( $tab['label'] ); ?></a>
								<?php
								$current++;
								endforeach;
							?>
