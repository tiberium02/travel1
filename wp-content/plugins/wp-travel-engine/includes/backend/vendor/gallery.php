...
<table class='form-table'>
	<tr>
		<td>
			<label for='wpte_gallery_id[enable]'><?php esc_html_e( 'Enable Gallery', 'wp-travel-engine' ); ?></label>
			...
			<div class="settings-note"><?php esc_html_e( 'Check this to enable gallery instead of featured image in single trip.', 'wp-travel-engine' ); ?></div>
		</td>
	</tr>
	<tr>
		<td>
			<a class='feat-img-gallery-add button' href='#' data-uploader-title='Add image(s) to gallery' data-uploader-button-text='Add image(s)'><?php esc_html_e( 'Add image(s)', 'wp-travel-engine' ); ?></a>
			...
			if ( $ids ) :
				...
				foreach ( $ids as $key => $value ) :
					...
					?>
				<li>
					<input type='hidden' name='wpte_gallery_id[<?php echo esc_attr( $key ); ?>]' value='<?php echo esc_attr( $value ); ?>'>
					<img class='image-preview' src='<?php echo esc_url( $image[0] ); ?>'>
					<a class='change-image button button-small' href='#' data-uploader-title='Change image' data-uploader-button-text='Change image'><?php esc_html_e( 'Change image', 'wp-travel-engine' ); ?></a><br>
					<small><a class='remove-image' href='#'><?php esc_html_e( 'Remove image', 'wp-travel-engine' ); ?></a></small>
				</li>
					<?php
				endforeach;
			endif;
...
function wpte_gallery_meta_save( $post_id ) {
	...
	if ( isset( $_POST['wpte_gallery_id'] ) ) {
		update_post_meta( $post_id, 'wpte_gallery_id', wte_clean( wp_unslash( $_POST['wpte_gallery_id'] ) ) );
	} else {
