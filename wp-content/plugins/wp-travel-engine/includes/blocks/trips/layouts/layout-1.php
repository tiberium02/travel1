					...
					<div class="category-trip-desti">
						...
						if ( wte_array_get( $settings, 'layoutFilters.showDuration', false ) && ( ! empty( $meta->duration['days'] ) || ! empty( $meta->duration['days'] ) ) ) :
							...
							<span class="category-trip-dur">
								...
								<span>
									<?php if ( ! empty( $meta->duration['days'] ) ) : ?>
										<span><?php printf( _n( '%d Day', '%d Days', $meta->duration['days'], 'wp-travel-engine' ), (float) $meta->duration['days'] ); ?></span>
									<?php endif; ?>
									<?php if ( false && ! empty( $meta->duration['nights'] ) ) : ?>
										<span><?php printf( _n( '%d Night', '%d Nights', $meta->duration['nights'], 'wp-travel-engine' ), (float) $meta->duration['nights'] ); ?></span>
									<?php endif; ?>
								</span>
							</span>
						<?php endif; ?>
					...
					switch ( $dates_layout ) {
						...
						case 'dates_list':
							...
							foreach ( $fsds as $fsd ) {
								...
								printf( '<li><span class="left">%1$s %2$s</span><span class="right">%3$s</span></li>',
									wte_esc_svg( $icon ),
