...
class TripSearch {
	...
	public static function search_filter_duration( $args ) {
		...
		$id    = wte_uniqid();
		?>
		<div class="wpte-trip__adv-field wpte__select-field" data-range-slider="#<?php echo esc_attr( $id ); ?>" data-range="<?php echo esc_attr( implode( ',', $range ) ); ?>" data-min="0" data-max="50" data-suffix="<?php esc_attr_e( 'Days', 'wp-travel-engine' ); ?>">
			<span class="icon">
	...
	public static function filter_trips_html( $post_data ) {
		...
		$foundposts = sprintf( _nx( '%s trip found', '%s trips found', $query->found_posts, 'number of trips', 'wp-travel-engine' ), number_format_i18n( $query->found_posts ) );
		return wp_send_json_success(
			array(
		...
		if ( isset( $max_duration ) && 0 != $max_duration ) {
			array_push(
				...
				array(
					...
					'value'   => array( $min_duration - 1, $max_duration + 1 ),
					'compare' => 'BETWEEN',
					'type'    => 'numeric',
				)
			);
	...
	public static function get_duration_range() {
		...
		if ( ! $range ) {
			$where   = $wpdb->prepare( 'meta_key = %s', 'wp_travel_engine_setting_trip_duration' );
	...
	public static function get_filters_sections() {
		...
		if ( ! empty( $settings['trip_search']['apply_in_search_page'] ) && '1' === $settings['trip_search']['apply_in_search_page'] ) {
			return array();
		}
		...
		return apply_filters(
			...
			array(
				...
				// 'group_size'     => array(
				// 'label'  => __( 'Group Size', 'wp-travel-engine' ),
				// 'show'   => empty( $settings['trip_search']['duration'] ),
				// 'render' => array( __CLASS__, 'filter_duration_render' ),
				// ),
				'activities'   => array(
				...
				// 'budget'       => array(
				// 'label'  => __( 'Budget', 'wp-travel-engine' ),
				// 'show'   => empty( $settings['trip_search']['budget'] ),
				// 'render' => array( __CLASS__, 'filter_budget_render' ),
				// ),
			)
		);
	}
		...
		if ( is_array( $terms ) && count( $terms ) > 0 ) {
			...
			foreach ( $terms as $term ) {
				...
				printf(
					...
					. '<span>%4$s</span>'
					. '</label>',
					checked( $term->slug, wte_array_get( $_GET, $term->taxonomy, false ), false ), // phpcs:ignore
					$term->slug,
					$term->taxonomy,
					$term->name
				);
	...
	public static function filter_duration_render( $filter ) {
		// phpcs:disable
		$min_duration = ! empty( $_GET['min-duration'] ) ? (int) $_GET['min-duration'] : '';
		$max_duration = ! empty( $_GET['max-duration'] ) ? (int) $_GET['max-duration'] : '';
		// phpcs:enable
		...
		echo '<div class="advanced-search-field search-duration search-trip-type" data-suffix="' . __( 'Days', 'wp-travel-engine' ) . '" data-range="' . $min_duration . ',' . $max_duration . '" data-range-slider="#duration-slider-range"><h3 class="filter-section-title">'
		. $filter['label'] . '</h3>'
		. '<div class="filter-section-content">'
			...
			. '<div class="duration-slider-value"><span id="min-duration" class="min-duration" name="min-duration">'
			. sprintf( __( '%1$s Days', 'wp-travel-engine' ), $min_duration )
			. '</span><span class="max-duration" id="max-duration" name="max-duration">'
			. sprintf( __( '%1$s Days', 'wp-travel-engine' ), $max_duration ) . '</span></div></div>'
		. '</div>';
	}
	...
	public static function filter_price_render( $filter ) {
		...
		print( '<div class="advanced-search-field search-cost search-trip-type" data-range="' . $min_cost . ',' . $max_cost . '" data-value-format="price" data-range-slider="#cost-slider-range">'
			...
			. '<div class="filter-section-content">'
				...
				. \wte_get_formated_price( $min_cost )
				. '</span><span class="max-cost">'
				. \wte_get_formated_price( $max_cost )
				. '</span></div>'
			. '</div>'
			. '</div>'
		);
	...
	public static function get_price_range() {
		...
		if ( ! $range ) {
			$where   = $wpdb->prepare( 'meta_key = %s', 'wp_travel_engine_setting_trip_actual_price' );
	...
	public static function search_form() {
		...
		$inline_script = 'jQuery(function($){
			var choices = {};
			$( "#sform-duration-slider-range" ).slider({
				range: true,
				min: ' . $duration_range->{'min_duration'} . ',
				max: ' . $duration_range->{'max_duration'} . ',
				values: [ ' . $duration_range->{'min_duration'} . ' , ' . $duration_range->{'max_duration'} . ' ],
			...
			$( "#sform-cost-slider-range" ).slider({
				range: true,
				min: ' . $cost_range->{'min_price'} . ',
				max: ' . $cost_range->{'max_price'} . ',
				values: [ ' . $cost_range->{'min_price'} . ', ' . $cost_range->{'max_price'} . ' ],
				slide: function( event, ui ) {
					$("#min-cost").text(ui.values[0]);
					$("#max-cost").text(ui.values[1]);
					var mincost = ui.values[0];
					var maxcost = ui.values[1];
					if ( "undefined" !== typeof WTE_CC_convData && WTE_CC_convData.reverse_rate ) {
					...
					$("#min-cost-value").val(mincost);
					$("#max-cost-value").val(maxcost);
				},
			});
		...
		wp_add_inline_script( 'wte-trip-search', $inline_script );

		ob_start();
			\wte_get_template( 'template-trip-search-form.php' );
		return ob_get_clean();
	}
