...
class WTE_Booking_Emails extends WP_Travel_Engine_Emails {
	...
	private function replace_content_tags( $content ) {
		...
		$billing_info = $this->booking->billing_info;

		$cart_info = (object) $this->booking->cart_info;

		$currency = $cart_info->currency;

		$trip = (object) array_shift( $order_trips );

		// Mapping Mail tags with values.
		$default_mail_tags = apply_filters(
			'wte_booking_mail_tags',
			array(
				'{trip_url}'                  => '<a href=' . esc_url( get_permalink( $trip->ID ) ) . '>' . esc_html( $trip->title ) . '</a>',
				'{name}'                      => wte_array_get( $billing_info, 'fname', '' ),
				'{fullname}'                  => wte_array_get( $billing_info, 'fname', '' ) . ' ' . wte_array_get( $billing_info, 'lname', '' ),
				'{user_email}'                => wte_array_get( $billing_info, 'email', '' ),
				'{billing_address}'           => wte_array_get( $billing_info, 'address', '' ),
				'{city}'                      => wte_array_get( $billing_info, 'city', '' ),
				'{country}'                   => wte_array_get( $billing_info, 'country', '' ),
				...
				'{price}'                     => wte_get_formated_price( $cart_info->total, $currency, '', ! 0 ),
				'{total_cost}'                => wte_get_formated_price( $cart_info->total, $currency, '', ! 0 ),
				'{due}'                       => wte_get_formated_price( $this->booking->due_amount, $currency, '', ! 0 ),
				'{sitename}'                  => get_bloginfo( 'name' ),
