...
<section class="featured-trip" id="featured_section">
	<div class="container">
		...
		<header class="section-header wow fadeInUp" data-wow-duration="1s" data-wow-delay="0.1s">
			<?php
			if ( $title ) {
				echo '<h2 class="section-title">' . esc_html( travel_agency_companion_get_featured_title() ) . '</h2>';
			}
			if ( $content ) {
				echo '<div class="section-content">' . wp_kses_post( travel_agency_companion_get_featured_content() ) . '</div>';
			}
			?>
		</header>
		<?php } ?>
							...
							<div class="holder">
								<div class="img-holder">
									...
									if ( function_exists( 'wte_get_trip' ) ) {
										...
										$trip->has_sale && print( '<span class="discount-holder"><span>' . sprintf( __( '%d&percnt; Off', 'travel-agency-companion' ), $trip->sale_percentage ) . '</span></span>' );
									} else {
